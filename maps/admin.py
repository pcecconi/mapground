...
from maps.models import Mapa, MapServerLayer, TMSBaseLayer
...
class MapServerLayerAdmin(admin.ModelAdmin):
    ...
    list_display = ['id', '__unicode__', 'capa', 'bandas', 'mapa', 'orden_de_capa', 'feature_info', 'archivo_sld']
...
class MapServerLayerInline(admin.TabularInline):
    ...
    extra = 2
    fields = ['capa', 'bandas', 'orden_de_capa', 'feature_info', 'capa_cantidad_de_archivos_sld', 'archivo_sld', 'capa_tipo_de_geometria']
...
class MapaAdminForm(forms.ModelForm):
    def clean(self):
        if self.instance.id is None:
            ...
            id_mapa = '{}_{}'.format(self.request.user.username, nombre)
...
class MapaAdmin(admin.ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        if obj.pk is None:
            ...
            obj.id_mapa = obj.slug = '{}_{}'.format(obj.owner.username, obj.nombre)
        obj.save()
